...
def braz_home(request):
    user = request.user

    try:
        dadosSetor = CustomGroupInfo.objects.get(nome='Brazlândia')
    ...
    superior = Group.objects.filter(id=46).first()
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=46).aggregate(
    ...
    if request.method == 'GET':
        sector_buttons = SectorButtons.objects.filter(group=46)
        context = {
...
@login_required(login_url="/login")
def new_request_braz(request):
    form = ModelFormMalotes()
    context = {'form': form, }

    return render(request, "pa/brazlandia/new_request_braz.html", context)


@login_required(login_url="/login")
def salvar_malote_braz(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            card.setor = get_object_or_404(Group, id=46)
            card.status = 'Triagem'
            card.save()
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=46),
                ...
                setor_atual="Brazlândia",  # Setor atual
            )
            history_entry.save()

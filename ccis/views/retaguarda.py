...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=18).aggregate(
...
def new_request_retaguarda(request):
    ...
    return render(request, "setores/retaguarda/new_request_retaguarda.html", context)


def salvar_malote_retaguarda(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            card.setor = get_object_or_404(Group, id=18)
            card.status = 'Triagem'
            card.save()
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=18),
            ...
            extra1 = request.POST["descricao-agendamento"]
            extra2 = request.POST["descricao-debito"]
            extra3 = request.POST["descricao-cheque"]
            extra4 = request.POST["descricao-conta"]
...
def request_ci_ret(request):
    ...
    return render(request, 'setores/retaguarda/new_request_retaguarda', {'form': form})


@login_required(login_url="/login")
def request_apontamentos_ret(request):
    descricao = ""
    if request.method == 'POST':
    ...
    return render(request, 'setores/retagurda/new_request_retaguarda.html', {'form': form, 'is_apontamento': True})

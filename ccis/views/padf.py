...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=20).aggregate(
...
@login_required(login_url="/login")
def new_request_padf(request):
    form = ModelFormPADFMalotes()
    context = {'form': form, }

    return render(request, "pa/padf/new_request_padf.html", context)


@login_required(login_url="/login")
def salvar_malote_padf(request):
    if request.method == 'POST':

        request.POST = request.POST.copy()  # Crie uma cópia do dicionário para modificação
        request.POST['assunto'] = 'Malote'

        form = ModelFormPADFMalotes(request.POST, request.FILES)

        if form.is_valid():
            card = form.save(commit=False)
            card.solicitante = request.user
            card.setor = get_object_or_404(Group, id=20)
            card.status = 'Triagem'
            card.save()
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=20),
            ...
            return redirect('padf_home')

    else:
        form = ModelFormPADFMalotes()

    return render(request, 'pa/padf/new_request_padf.html', {'form': form})


@login_required(login_url="/login")
def processos_padf(request):

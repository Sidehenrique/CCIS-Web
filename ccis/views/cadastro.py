...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=7).aggregate(
...
def new_request_cadastro(request):
    ...
    return render(request, "setores/cadastro/new_request_cadastro.html", context)


def salvar_malote_cadastro(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=7),
...
def request_ci_cad(request):
    ...
    return render(request, 'setores/cadastro/new_request_cadastro', {'form': form})


@login_required(login_url="/login")
def request_apontamentos_cad(request):
    descricao = ""
    if request.method == 'POST':
    ...
    return render(request, 'setores/cadastro/new_request_cadastro.html', {'form': form, 'is_apontamento': True})

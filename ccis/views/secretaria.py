...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=31).aggregate(
...
def new_request_secretaria(request):
    ...
    return render(request, "setores/secretaria/new_request_secretaria.html", context)
...
def salvar_malote_secretaria(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=31),
                ...
                setor_atual="Secretaria",  # Setor atual
            )
            history_entry.save()
    ...
    return render(request, 'setores/secretaria/new_request_secretaria.html', {'form': form})


@login_required(login_url="/login")
def request_ci_sec(request):
    if request.method == 'POST':
    ...
    return render(request, 'setores/secretaria/new_request_secretaria', {'form': form})


@login_required(login_url="/login")
def request_apontamentos_sec(request):
    descricao = ""
    if request.method == 'POST':
    ...
    return render(request, 'setores/secretaria/new_request_secretaria.html', {'form': form, 'is_apontamento': True})

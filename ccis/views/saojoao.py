...
def SJ_home(request):
    ...
    try:
        dadosSetor = CustomGroupInfo.objects.get(nome='São João')
    ...
    superior = Group.objects.filter(id=22).first()
...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=22).aggregate(
    ...
    if request.method == 'GET':
        sector_buttons = SectorButtons.objects.filter(group=22)
        context = {
...
def salvar_malote_SJ(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            card.setor = get_object_or_404(Group, id=22)
            card.status = 'Triagem'
            card.save()
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=22),
                ...
                setor_atual="São João",  # Setor atual
            )
            history_entry.save()

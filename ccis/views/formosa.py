...
from ..forms import ModelFormFormosaMalotes
from django.db.models import OuterRef, Subquery,F, ExpressionWrapper, fields,Avg, Count,FloatField


@login_required(login_url="/login")
def formosa_home(request):
    user = request.user

    try:
        dadosSetor = CustomGroupInfo.objects.get(nome='Formosa')
    ...
    superior = Group.objects.filter(id=21).first()
...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=21).aggregate(
    ...
    if request.method == 'GET':
        sector_buttons = SectorButtons.objects.filter(group=21)
        context = {
...
@login_required(login_url="/login")
def new_request_formosa(request):
    form = ModelFormFormosaMalotes()
    context = {'form': form, }

    return render(request, "pa/formosa/new_request_formosa.html", context)


@login_required(login_url="/login")
def salvar_malote_formosa(request):
    if request.method == 'POST':

        request.POST = request.POST.copy()  # Crie uma cópia do dicionário para modificação
        request.POST['assunto'] = 'Malote'

        form = ModelFormFormosaMalotes(request.POST, request.FILES)
        ...
        if form.is_valid():
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=21),
                ...
                setor_atual="Formosa",  # Setor atual
            )
            history_entry.save()
            ...
            return redirect('formosa_home')

    else:
        form = ModelFormFormosaMalotes()

    return render(request, 'pa/formosa/new_request_formosa.html', {'form': form})

...
def saosebas_home(request):
    ...
    try:
        dadosSetor = CustomGroupInfo.objects.get(nome='São Sebastiao')
...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=26).aggregate(
...
def new_request_saosebas(request):
    ...
    return render(request, "pa/saosebas/new_request_saosebas.html", context)
...
def salvar_malote_saosebas(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            card.setor = get_object_or_404(Group, id=26)
            card.status = 'Triagem'
            card.save()
            ...
            history_entry = CardSetorHistory(
                card=card,
                setor=get_object_or_404(Group, id=26),
    ...
    return render(request, 'pa/saosebas/new_request_saosebas.html', {'form': form})


@login_required(login_url="/login")
def processos_SB(request):

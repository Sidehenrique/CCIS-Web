...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=27).aggregate(
...
def salvar_malote_credito(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            card.setor = get_object_or_404(Group, id=27)
            card.status = 'Triagem'
            card.save()
...
def request_ci_cre(request):
    ...
    return render(request, 'setores/credito/new_request_credito', {'form': form})
...
def request_apontamentos_cre(request):
    ...
    return render(request, 'setores/credito/new_request_credito.html', {'form': form, 'is_apontamento': True})

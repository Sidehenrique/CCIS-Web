...
def controladoria_home(request):
    user = request.user

    try:
        dadosSetor = CustomGroupInfo.objects.get(nome='Performance Corporativa')
...
# Contagem dos processos que estão em triagem e em andamento no setor
    ...
    contagem_condicional = CardSetorHistory.objects.filter(
        ...
        setor_atual='Performance Corporativa',
...
# média da avaliação por setor
    ...
    media_grupo_2 = OperatorRating.objects.filter(group_id=1).aggregate(
...
def request_ci_pc(request):
    ...
    return render(request, 'setores/controladoria/new_request_PC', {'form': form})


@login_required(login_url="/login")
def request_apontamentos_pc(request):
    descricao = ""
    if request.method == 'POST':
    ...
    return render(request, 'setores/controladoria/new_request_PC.html', {'form': form, 'is_apontamento': True})


@login_required(login_url="/login")
def processos_PC(request):
